...
import { CHANNEL_BUDGETS, PERMISSION_ROWS } from "@/data/mockAdmin";
...
function PermissionCell({ level }: { level: PermissionLevel }) {
  ...
  return (
    <span
      title={meta.label}
      className={cn("mx-auto flex h-7 w-7 items-center justify-center rounded-full", meta.className)}
    >
  ...
  return (
    <div className="mx-auto flex max-w-[1600px] flex-col gap-6">
      <PageHeader
        ...
        subtitle="Theo dõi ngân sách Marketing theo kênh và phân quyền truy cập hệ thống."
      />

      <div className="flex w-fit rounded-[var(--radius-control)] bg-gray-100 p-1">
        {(
          [
      ...
      {tab === "budget" ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Quyền truy cập từng module theo vai trò trong hệ thống</CardDescription>
          </CardHeader>
            ...
            <table className="w-full min-w-[640px] border-collapse text-left">
              <thead>
                <tr className="border-b border-[var(--color-border)] text-[11px] uppercase tracking-wide text-[var(--color-text-secondary)]">
                  ...
                  <th scope="col" className="pb-2 text-center font-semibold">Giáo viên</th>
                </tr>
              </thead>
              <tbody>
                {PERMISSION_ROWS.map((row) => (
                  <tr key={row.id} className="border-b border-[var(--color-border)] text-[13px] last:border-0 hover:bg-gray-50">
                    <td className="py-3 font-medium text-[var(--color-text)]">{row.module}</td>
                    <td className="py-3 text-center"><PermissionCell level={row.superAdmin} /></td>
                    <td className="py-3 text-center"><PermissionCell level={row.marketing} /></td>
                    <td className="py-3 text-center"><PermissionCell level={row.cskh} /></td>
                    <td className="py-3 text-center"><PermissionCell level={row.teacher} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
